...
>(src: NdTensorView<T, 3>, chan_weights: [f32; CHANS]) -> NdTensor<f32, 3> {
    panic!("STUB: not implemented");
}
...
mod tests {
    ...
    fn test_image_source_from_bytes() {
        ...
        for Case { len, width, height, error } in cases {
    ...
    fn test_image_source_from_data() {
        ...
        for Case { shape, error, order } in cases {
    ...
    fn test_prepare_image_rgb_rgba_u8() {
        ...
        for Case { data, shape, order, rgb: [r, g, b] } in cases {
            let tensor = NdTensor::from_data(shape, data);
            let source = ImageSource::from_tensor(tensor.view(), order).unwrap();
            let result = prepare_image(source);
            assert_eq!(result.shape(), [1, 1, 1]);
            let expected = expected_grey_from_rgb(
                r as f32 / 255.0,
                g as f32 / 255.0,
                b as f32 / 255.0,
            );
    ...
    fn test_prepare_image_multi_pixel_rgb() {
        ...
        let cases = [
            ...
            Case {
                #[rustfmt::skip]
                data: vec![255, 0, 0, 128, 0, 255, 0, 128, 0, 0, 255, 128,],
                shape: [3, 2, 2],
                order: DimOrder::Chw,
            },
        ];
        ...
        let expected_grey = expected_grey_from_rgb(
            128.0 / 255.0,
            128.0 / 255.0,
            128.0 / 255.0,
        );
