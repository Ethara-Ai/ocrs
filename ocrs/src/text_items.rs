...
impl TextLine {
    ...
    pub fn words(&self) -> impl Iterator<Item = TextWord<'_>> {
        panic!("STUB: not implemented");
        #[allow(unreachable_code)] std::iter::empty::<TextWord<'_>>()
    }
}
...
mod tests {
    ...
    fn test_item_rotated_rect() {
        ...
        assert_eq!(word.bounding_rect(), Rect::from_tlhw(0, 0, 25, char_width * 3));
        ...
        assert_eq!(
            word.rotated_rect().corners(), [(25, 30), (25, 0), (0, 0), (0, 30)].map(| (y,
            x) | Point::from_yx(y as f32, x as f32))
        );
    }
    #[test]
    fn test_line_words() {
        ...
        assert_eq!(words[0].bounding_rect(), Rect::from_tlhw(0, 0, 25, char_width * 3));
        assert_eq!(words[1].to_string(), "bar");
        assert_eq!(
            words[1].bounding_rect(), Rect::from_tlhw(0, char_width * 4, 25, char_width *
            3)
        );
        assert_eq!(words[2].to_string(), "baz");
        assert_eq!(
            words[2].bounding_rect(), Rect::from_tlhw(0, char_width * 9, 25, char_width *
            3)
        );
    }
}
